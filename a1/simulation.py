...
class GroceryStoreSimulation:
    ...
    def run(self, initial_events: list[Event]) -> None:
        ...
        # Process events until there are no more events left
        while not self._events.is_empty():
            # Get the next event from the event queue
            event = self._events.remove()

            # Process the event and find out what new events it generates
            new_events = event.do(self._store)
            # Add any new events generated by processing the current event
            for new_event in new_events:
                self._events.add(new_event)

            # Update statistics based on the event, make sure name isn't already in our list
            if isinstance(event, CustomerArrival):
                if event.customer not in arrived_customers:
                    arrived_customers.append(event.customer)
